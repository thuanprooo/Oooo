...
bot.onText(/\/attack (https?:\/\/[^\s]+) (\d+)/, (msg, match) => {
  ...
  if (!ADMIN_IDS.includes(userId)) {
    ...
    const last Time = lastUserAttackTime[userId] || 0;
...
const ADMIN_IDS = [7818408538]; // thay bằng id bạn

// /stop command: dừng polling và thoát node sau khi send message
bot.onText(/\/stop/, (msg) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  ...
  bot.sendMessage(chatId, "🛑 Bot sẽ dừng...").then(() => {
    // dừng polling (ngay lập tức)
    bot.stopPolling(true); // true để clear timers/listeners nội bộ
    // nếu muốn thoát tiến trình Node:
    process.exit(0);
  }).catch(err => {
    console.error('Err when sending stop msg:', err);
    // vẫn cố gắng dừng
